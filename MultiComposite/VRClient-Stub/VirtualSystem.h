...
#include "VirtualSystem/VirtualSystem_011.h"
#include "VirtualSystem/VirtualSystem_012.h"
#include "VirtualSystem/VirtualSystem_014.h"
#include "VirtualSystem/VirtualSystem_015.h"
#include "VirtualSystem/VirtualSystem_016.h"
#include "VirtualSystem/VirtualSystem_017.h"
#include "VirtualSystem/VirtualSystem_019.h"
#include "VirtualSystem/VirtualSystem_020.h"
#include "VirtualSystem/VirtualSystem_021.h"
...
public:
	...
	VirtualSystem()
	{
		virtualSystem = nullptr;
		steamSystem = nullptr;
	}

	~VirtualSystem()
	{
		delete virtualSystem;
		delete steamSystem;
	}
	...
	* actual tracked device. */
	...
	bool GetBoolTrackedDeviceProperty(vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError* pErrorL);

	/** Returns a float property. If the device index is not valid or the property is not a float type this function will return 0. */
	float GetFloatTrackedDeviceProperty(vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError* pErrorL);

	/** Returns an int property. If the device index is not valid or the property is not a int type this function will return 0. */
	int32_t GetInt32TrackedDeviceProperty(vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError* pErrorL);

	/** Returns a uint64 property. If the device index is not valid or the property is not a uint64 type this function will return 0. */
	uint64_t GetUint64TrackedDeviceProperty(vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError* pErrorL);

	/** Returns a matrix property. If the device index is not valid or the property is not a matrix type, this function will return identity. */
	HmdMatrix34_t GetMatrix34TrackedDeviceProperty(vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, ETrackedPropertyError* pErrorL);
	...
	* greater than the returned size and pBuffer is non-NULL, pBuffer is filled with the contents of array of properties. */
	uint32_t GetArrayTrackedDeviceProperty(vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, PropertyTypeTag_t propType, void* pBuffer, uint32_t unBufferSize, ETrackedPropertyError* pError);

	/** Returns a string property. If the device index is not valid or the property is not a string type this function will
	* return 0. Otherwise it returns the length of the number of bytes necessary to hold this string including the trailing
	* null. Strings will always fit in buffers of k_unMaxPropertyStringSize characters. */
	uint32_t GetStringTrackedDeviceProperty(vr::TrackedDeviceIndex_t unDeviceIndex, ETrackedDeviceProperty prop, VR_OUT_STRING() char* pchValue, uint32_t unBufferSize, ETrackedPropertyError* pErrorL);
	...
	* system-related functionality is consuming the input stream. */
	...
	/** Returns true virtualVR is drawing controllers on top of the application. Applications should consider
	* not drawing anything attached to the user's hands in this case. */
	bool IsSteamVRDrawingControllers();

	/** Returns true if the user has put virtualVR into a mode that is distracting them from the application.
	* For applications where this is appropriate, the application should pause ongoing activity. */
	bool ShouldApplicationPause();

	/** Returns true if virtualVR is doing significant rendering work and the game should do what it can to reduce
	...
	* needed to hold the list. */
	...
	/** Returns the current version of the virtualVR runtime. The returned string will remain valid until VR_Shutdown is called.
	*
	* NOTE: Is it not appropriate to use this version to test for the presence of any virtualVR feature. Only use this version
...
public:
	...
	HmdMatrix44_t GetProjectionMatrix(EVREye eEye, float fNearZ, float fFarZ, EGraphicsAPIConvention convention);
